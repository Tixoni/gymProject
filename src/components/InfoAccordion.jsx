...
export default function InfoAccordion({ title = 'Справка', children }) {
  ...
  return (
    <section className="rounded-xl border border-zinc-800/90 bg-zinc-950/25">
      <button
        type="button"
        className="flex w-full items-center justify-between gap-3 rounded-xl px-4 py-3 text-left transition hover:bg-zinc-900/25"
      ...
      >
        ...
        <span
          className={`inline-flex h-7 w-7 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900/10 text-zinc-300 transition ${
            open ? 'rotate-180' : ''
          }`}
        >
          ▾
